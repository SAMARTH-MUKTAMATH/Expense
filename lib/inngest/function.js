...
export const notifyBudgetThreshold = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    return await step.run("send-if-still-over", async () => {
      ...
      // Also deliver via Web Push so the user sees it instantly on their
      // phone (if they've subscribed). Failures here don't block the email
      // path — they're best-effort.
      try {
        await sendPushToUser(budget.userId, {
          title: `Budget alert: ${account.name}`,
          body: `You've used ${percentageUsed.toFixed(0)}% of your monthly budget.`,
          url: "/dashboard",
          tag: "budget-alert",
        });
      } catch (e) {
        console.warn("[notifyBudgetThreshold] push send failed:", e);
      }
...
// 6. TEST: Push every 2 minutes — for verifying Web Push end-to-end without
// having to trip a real budget threshold. Sends a "ping" notification to
// every user that has at least one push subscription.
// Remove this function (and unregister it in app/api/inngest/route.js) once
// you've confirmed push delivery on your devices.
export const testPushEvery2Min = inngest.createFunction(
  {
    id: "test-push-every-2min",
    name: "TEST: Push every 2 minutes",
    triggers: [{ cron: "*/2 * * * *" }],
  },
  async ({ step }) => {
    const userIds = await step.run("fetch-subscribed-users", async () => {
    ...
    if (userIds.length === 0) return { delivered: 0, users: 0 };

    let delivered = 0;
    for (const userId of userIds) {
      const r = await step.run(`push-${userId}`, async () => {
        return await sendPushToUser(userId, {
          title: "BudgetFLOW · test ping",
          body: `Push works! Server time ${new Date().toLocaleTimeString("en-IN")}`,
          url: "/dashboard",
          tag: "test-ping",
        });
      });
    ...
    return { delivered, users: userIds.length };
  }
);